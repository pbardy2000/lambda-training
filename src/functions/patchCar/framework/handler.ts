...
import { patchCar } from "../application/service/patch-car";

export async function handler(event: APIGatewayProxyEvent) {
	try {
		bootstrapLogging("patch-car", event);
		...
		if (!validateCar(body)) {
			...
			return createResponse(
				{ msg: "Event body is malformed" },
				HttpStatus.BAD_REQUEST,
			);
		}

		const id = Number.parseInt(String(event.pathParameters.value), 10);
		...
		const updated = await patchCar(id, body);
